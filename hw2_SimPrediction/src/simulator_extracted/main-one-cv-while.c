...
void do_fill(int value) {
    ...
    ensure(buffer[fill_ptr] == EMPTY, "error: tried to fill a non-empty buffer"); //fill_ptr is just a index into the array, it point a position/a slot to put item in the buffer
    buffer[fill_ptr] = value;
    fill_ptr = (fill_ptr + 1) % max; //R. for circular array
    num_full++;
}

int do_get() {
    int tmp = buffer[use_ptr];
    ensure(tmp != EMPTY, "error: tried to get from an empty buffer slot"); // R. err sentence modifed by Rich
