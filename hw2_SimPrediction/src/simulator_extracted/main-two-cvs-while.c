...
pthread_cond_t empty  = PTHREAD_COND_INITIALIZER;   // R. the condition that the buffer is empty
pthread_cond_t fill   = PTHREAD_COND_INITIALIZER;   //R. the buffer is full
...
void *producer(void *arg) {
	...
	Mutex_lock(&m);             p1;
	while (num_full == max) {   p2;    // need that predictate here to really see if we need to wait(similar to int is_done in the example i saw
